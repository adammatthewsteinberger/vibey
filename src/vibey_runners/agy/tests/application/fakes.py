...
ever calls time.sleep() for real."""
...
def rpm_window_signals() -> TurnSignals:
    return TurnSignals(
        http_status=429,
        status="RESOURCE_EXHAUSTED",
...
def window_exhausted_signals(*, resets_at: datetime | None = None) -> TurnSignals:
    if resets_at is not None:
        ...
        # so tests that need a known deadline pass it via a custom TurnSignals
        # that classify maps to WindowExhausted(rpd). For RPM the classifier
        # ignores resets_at; callers that need a typed window should use
        # rpm_window_signals().
        del resets_at
    return TurnSignals(
        http_status=429,
...
def auth_failed_signals() -> TurnSignals:
    return TurnSignals(http_status=401, status="UNAUTHENTICATED", message="bad key")
