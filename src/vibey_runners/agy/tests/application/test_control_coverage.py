...
async def test_null_savepoint_store_unwind_raises() -> None:
    from agyloop.application.runner import _NullSavePointStore
...
async def test_null_notifier_control_events_lock() -> None:
    from agyloop.application.runner import (
        _NullEventSink,
        _NullNotifier,
        _NullRunControl,
        _NullSessionLock,
        ...
        agent_gateway=FakeAgentGateway(
            [
                ScriptedTurn(
                    signals=available_signals(),
                    verdict=None,
                    output_text="AGYLOOP_TASK_FULLY_COMPLETE",
                )
            ]
        ),
        capacity_probe=FakeCapacityProbe([available_signals()]),
...
async def test_null_state_bus_and_publish_via_runner() -> None:
    from agyloop.application.runner import _NullStateBus
...
async def test_set_preset_applied_before_next_turn() -> None:
    from agyloop.domain.control import SetPresetCommand
    from agyloop.domain.model_profile import DEFAULT_MODEL_HIGH
...
async def test_auto_downgrade_budget_queues_low() -> None:
    from agyloop.domain.model_profile import ModelAliases, profile_for_preset
...
async def test_null_stream_ui_methods() -> None:
    from agyloop.application.runner import _NullStreamUi
