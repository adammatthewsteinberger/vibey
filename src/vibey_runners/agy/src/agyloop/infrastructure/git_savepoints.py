...
"""Git-backed save points under refs/agyloop/<run_id>/<n>."""
...
class GitSavePointStore:
    ...
    def create(
        ...
        *,
        ...
        if has_staged:
            if message and attempt is None and not summary and not remaining_work:
                subject = f"chore(agyloop): {message}"
                body = f"Run: {run_id}\nLabel: {label}\n"
            else:
    ...
    def unwind(self, *, run_id: str, to: str, backup: bool) -> UnwindResult:
        ...
        if backup:
            ...
            backup_ref = f"refs/agyloop/backup/{run_id}/{stamp}"
