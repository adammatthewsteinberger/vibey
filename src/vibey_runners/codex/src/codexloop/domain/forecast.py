...
from codexloop.domain.capacity import Available, RateLimitWindow
...
def _window_headroom(
    window: RateLimitWindow | None,
    *,
    label: str,
    as_of: datetime | None,
    fallback_resets_at: datetime | None = None,
) -> Headroom:
    if window is None or window.remaining_percent is None:
        return Headroom(UNKNOWN, label, as_of, fallback_resets_at)
    resets = window.resets_at or fallback_resets_at
    return Headroom(window.remaining_percent / 100.0, label, as_of, resets)
...
def _vendor_headrooms(
    ...
    *,
    ...
    """Vendor-reported headroom, from the plan windows the app-server reports.

    ``remaining_percent`` is a real number from a real RPC rather than
    something inferred, which makes this the strongest forecast of the four
    runners.
    """
    del now
    windows = available.plan_windows
    if windows is None:
        return (Headroom(UNKNOWN, "window:primary", as_of, resets_at),)
    return (
        _window_headroom(
            windows.primary, label="window:primary", as_of=as_of, fallback_resets_at=resets_at
        ),
        _window_headroom(
            windows.secondary, label="window:secondary", as_of=as_of, fallback_resets_at=resets_at
        ),
    )
