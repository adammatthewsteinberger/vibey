...
from codexloop.domain.capacity import Available, PlanWindows, RateLimitWindow
...
def available_at(used: float) -> Available:
    """`used` is the fraction consumed: 0.99 means 99% used, 1% headroom."""
    return Available(
        plan_windows=PlanWindows(
            primary=RateLimitWindow(used_percent=used * 100.0, window_minutes=300, resets_at=None),
            secondary=None,
            plan_type="pro",
            limit_reached=None,
        )
